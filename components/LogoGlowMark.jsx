/**
 * 3D-glow ANX mark — an SVG recreation of the glowing/depth logo video:
 * a glossy extruded logo, a soft blurred drop shadow, diagonal light
 * trails that sweep across the surface on a loop, and twinkling sparkles.
 *
 * Props:
 *  - className / style: sizing & layout, forwarded to the <svg>
 *  - animated: set false (or reduced motion is on) to freeze the sweep/twinkle
 *  - gradientId: unique id so multiple instances on a page don't clash
 */
export function LogoGlowMark({ className, style, animated = true, gradientId = "glow-mark" }) {
  const on = animated;
  const g = (name) => `${gradientId}-${name}`;
    ...
    <svg viewBox="-20 -20 240 240" className={className} style={style} aria-hidden="true">
      <defs>
        <linearGradient id={g("fill")} x1="20" y1="200" x2="200" y2="10" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="var(--violet-deep, #4C0E8F)" />
          <stop offset="45%" stopColor="var(--primary, #8B2FD1)" />
          <stop offset="75%" stopColor="var(--magenta, #C026D3)" />
          <stop offset="100%" stopColor="var(--amber, #F59E0B)" />
        </linearGradient>
      ...
      {/* soft drop shadow, offset down-right, matching the 3D depth in the video */}
      <g
      ...
      {/* ambient colour glow behind the mark */}
      <g
        stroke={`url(#${g("fill")})`}
        strokeWidth="22"
        fill="none"
...
function Sparkle({ x, y, delay = "0s", scale = 1, animated }) {
  const s = 7 * scale;
  return (
